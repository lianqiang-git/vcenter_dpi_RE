...
import com.ghca.vm.exception.VcenterException;
import com.ghca.common.util.StringUtil;
import com.ghca.vm.util.Pair;
import com.ghca.vm.util.VmwareContext;
import com.ghca.vm.util.VmwareHelper;
...
public class HypervisorHostHelper {
    ...
    private static void setupPVlanPair(DistributedVirtualSwitchMo dvSwitchMo, ManagedObjectReference morDvSwitch, Integer vid, Integer spvlanid, String pvlanType) throws Exception {
        ...
        Map<Integer, PvlanType> vlanmap = dvSwitchMo.retrieveVlanPvlan(vid, spvlanid, morDvSwitch);
        if (!vlanmap.isEmpty()) {
            // Then either vid or pvlanid or both are already being used. Check how.
            // First the primary pvlan id.
            if (vlanmap.containsKey(vid) && !vlanmap.get(vid).equals(PvlanType.promiscuous)) {
            ...
            if (spvlanid.equals(vid)) {
                if (vlanmap.containsKey(spvlanid) && !vlanmap.get(spvlanid).equals(PvlanType.promiscuous)) {
