...
public class DatastoreVimBase   {
    private com.ghca.vm.util.ConnectedVimServiceBase connectedVimServiceBase;

    public com.ghca.vm.util.ConnectedVimServiceBase getConnectedVimServiceBase() {
        return connectedVimServiceBase;
    }

    public void setConnectedVimServiceBase(com.ghca.vm.util.ConnectedVimServiceBase connectedVimServiceBase) {
        this.connectedVimServiceBase = connectedVimServiceBase;
    }

    public DatastoreVimBase(com.ghca.vm.util.ConnectedVimServiceBase connectedVimServiceBase) {
        this.connectedVimServiceBase = connectedVimServiceBase;
    }
     ...
     * 扩容vmfs datastore
     */
   /* public void expandVmfsDatastore(String datastorename,String newdatastorename) throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg, InvalidNameFaultMsg, DuplicateNameFaultMsg {
        ManagedObjectReference morHost = new ManagedObjectReference();
        morHost.setType("HostSystem");
        morHost.setValue("host-48");//设置

        ManagedObjectReference storageResourceManager =
                connectedVimServiceBase.serviceContent.manager();
        ManagedObjectReference datastore=getDatastoreByName(datastorename);
        connectedVimServiceBase.vimPort.expandVmfsDatastore(storageResourceManager,datastore,newdatastorename);
    }*/
}
